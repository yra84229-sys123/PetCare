...
const AdminLayout = () => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      if (window.PetCareDB) {
        const sessionUser = await window.PetCareDB.auth.checkSession(["admin"]);
        if (sessionUser) {
          setUser(sessionUser);
        } else {
  ...
  const initials = user.name ? user.name.split(" ").map(n => n[0]).join("").toUpperCase().substring(0, 2) : "AM";
    ...
    <div className="dashboard-wrapper admin-theme">
      ...
      <aside className={`sidebar ${isSidebarOpen ? 'active' : ''}`}>
        ...
        <ul className="sidebar-menu">
          ...
          <li>
            <NavLink to="/admin/pets" className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}>
              <i className="fas fa-dog"></i> Manage Pets
            </NavLink>
          </li>
          <li>
            <NavLink to="/admin/owners" className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}>
              <i className="fas fa-users"></i> Manage Owners
            </NavLink>
          </li>
          <li>
          ...
          <li>
            <NavLink to="/admin/users" className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}>
              <i className="fas fa-user-shield"></i> Manage Users
            </NavLink>
          </li>
        ...
        <header className="dashboard-header">
          <div className="dashboard-title">
            ...
            <div>
              ...
              <p id="page-desc">Welcome back to your administration command center.</p>
            </div>
          </div>
